...
static unsigned int mars3_sockets_bitmap = 1;
...
DEFINE_STATIC_KEY_FALSE(gic_nonsecure_priorities_2500);
EXPORT_SYMBOL(gic_nonsecure_priorities_2500);
 ...
 *
 * If both are true (which is when gic_nonsecure_priorities_2500 gets enabled),
...
#define GICD_INT_RPR_PRI(priority)					\
	({								\
		...
		if (static_branch_unlikely(&gic_nonsecure_priorities_2500))	\
...
static void __init gic_dist_init(void)
{
	...
	for (skt = 0; skt < MAX_MARS3_SOC_COUNT; skt++) {
		if (((1 << skt) & mars3_sockets_bitmap) == 0)
			continue;
...
static void gic_enable_nmi_support(void)
{
	...
	if (gic_has_group0() && !gic_dist_security_disabled())
		static_branch_enable(&gic_nonsecure_priorities_2500);
...
static int __init gic_of_init(struct device_node *node, struct device_node *parent)
{
	...
	mars3_gic_dists[0].phys_base = dist_phys_base;
